...
func TestFinalizeRejectsMissingRouteSetCandidate(t *testing.T) {
	y := "tun:\n  route-address-set:\n    - missing\n"
	if _, err := FinalizeForIOS(y, `{}`); err == nil {
		t.Fatal("expected missing materialized provider error")
	}
}

func TestFinalizeRejectsNonIPCidrRouteSet(t *testing.T) {
	y := `
rule-providers:
...
`
	if _, err := FinalizeForIOS(y, `{}`); err == nil || !strings.Contains(err.Error(), "ipcidr") {
		t.Fatalf("non-ipcidr route set was accepted: %v", err)
	}
}

...
//
// Firing is OPT-IN, which is the one deliberate difference from upstream. Upstream's
// killerDisabled defaults to acting; ours defaults to reporting. The reason is that the action is
// closing every tracked connection, and the machine's thresholds have never run against a real
// device's memory behaviour in this codebase -- an earlier version of this repository shed on
// every OS notification and the measured result was that it freed almost nothing while killing
// every app's session. Report-only makes the first device run produce the numbers that justify
// turning it on, rather than making users the experiment.
...
func startPressureThresholdMonitor(softLimit int64, shedEnabled bool) {
	...
	wake, stop, machine := pressureThresholdWake, pressureThresholdStop, pressureThresholdMachine
	...
	go runPressureThresholdLoop(machine, wake, stop)
}

func runPressureThresholdLoop(machine *pressureMachine, wake <-chan struct{}, stop <-chan struct{}) {

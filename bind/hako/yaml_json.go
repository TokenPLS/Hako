...
func YamlToJSON(rawYAML string) (*StringBox, error) {
	...
	if err := decoder.Decode(&document); err != nil {
		return nil, fmt.Errorf("hako: decode YAML: %w", err)
	}
	...
	if len(document.Content) != 1 || document.Content[0].Kind != yaml.MappingNode {
		return nil, fmt.Errorf("hako: YAML configuration root must be an object")
	}
	...
	if err := document.Content[0].Decode(&safety); err != nil {
		return nil, fmt.Errorf("hako: reject unsafe YAML configuration: %w", err)
	}
...
func JSONToYaml(rawJSON string) (*StringBox, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("hako: decode JSON: %w", err)
	}
	if _, err := decoder.Token(); err != io.EOF {
		if err != nil {
			return nil, fmt.Errorf("hako: decode trailing JSON: %w", err)
		}
		return nil, fmt.Errorf("hako: JSON configuration must contain exactly one value")
	}
	if rootNode.Kind != yaml.MappingNode {
		return nil, fmt.Errorf("hako: JSON configuration root must be an object")
	}
	...
	if err := encoder.Encode(&document); err != nil {
		return nil, fmt.Errorf("hako: encode JSON configuration as YAML: %w", err)
	}
	if err := encoder.Close(); err != nil {
		return nil, fmt.Errorf("hako: finish YAML encoding: %w", err)
	}

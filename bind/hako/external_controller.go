...
func applyExternalController(cfg *config.Config) {
	...
	if len([]byte(path)) > clashAPIMaxUnixPathBytes {
		log.Errorln("[iOS] Clash API Unix path is %d bytes; Darwin limit is %d: %s",
			len([]byte(path)), clashAPIMaxUnixPathBytes, path)
	}
	if probe := controllerServerConfig(cfg, path); probe.Addr == "" && probe.TLSAddr == "" {
		// Nothing configured: leave the socket as Setup created it rather than tearing down and
		// rebuilding a listener the app is already talking to.
		return
	}

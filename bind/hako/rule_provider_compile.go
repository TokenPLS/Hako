...
func CompileRuleProvider(sourcePath, behavior, format, outputPath string) (*RuleProviderCompileResult, error) {
	...
	if !providerSourceContained(sourcePath, home) {
		return nil, errors.New("hako: rule provider source is outside this app's container")
	}
	if !providerSourceContained(outputPath, home) {
		return nil, errors.New("hako: compiled rule provider destination is outside this app's container")
	}
	source, err := readBoundedRegularFile(sourcePath, int64(maximumProviderResourceBytes), "rule provider")
	if err != nil {
		return nil, fmt.Errorf("hako: read rule provider: %w", err)
	}
	...
	if err := writeRuntimeProviderFile(outputPath, result.artifact); err != nil {
		return nil, fmt.Errorf("hako: write compiled rule provider: %w", err)
	}
...
// ruleProviderCompileRefusal answers what compileRuleProviderPayload would
// refuse, without building anything: a streaming scan with nothing
// accumulated, cheap enough for the extension. It exists because the refusal
// verdict must survive a manifest invalidation -- an upgrade voids the
// manifest, the client republishes only on activation, and the extension
// restages without compiling; judging here keeps the empty-set ruling in
// force instead of hardlinking the refused source back online.
//
// It answers "" for domain/ipcidr sources on purpose: their only refusals
// come out of the real converter (malformed content), and a malformed set
// riding as text fails inside upstream's Initial() to an empty matcher --
// warn-and-continue, no big table ever built -- so the miss is safe and the
// next compiling publish records the real verdict.
func ruleProviderCompileRefusal(source []byte, behavior, format string) string {
	...
	case "domain", "ipcidr":
		return ""
	case "classical":
		_, reason := classicalDomainScan(source, nil)
		return reason
	default:
		return fmt.Sprintf("unknown behavior %q", behavior)
	}
}

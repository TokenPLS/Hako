...
func TestParseConfigForIOSRejectsUnsafeProviderHealthCheckDurations(t *testing.T) {
	tests := map[string]string{
		...
		"overflowing timeout":  "timeout: 9223372036855",
	}
...
func TestParseConfigForIOSRejectsUnsafeProxyGroupDurations(t *testing.T) {
	tests := map[string]string{
		"negative interval":    "interval: -1",
		"overflowing interval": "interval: 9223372037",
		"negative timeout":     "timeout: -1",
...
rules:
  - MATCH,TEST
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxy-groups[0]") {
				t.Fatalf("unsafe proxy-group duration error = %v", err)
			}
		})
	}
}
...
func TestParseConfigForIOSRejectsUnsafeHysteriaHopInterval(t *testing.T) {
...
rules:
  - MATCH,HY
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0].hop-interval") {
				t.Fatalf("unsafe Hysteria hop interval error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeInlineProviderHysteriaHopInterval(t *testing.T) {
	setupConfigPipelineTest(t)
	_, err := parseConfigForIOS(`
proxy-providers:
  inline:
    ...
    payload:
      - name: HY
        ...
        hop-interval: -1
proxy-groups:
  - {name: SELECT, type: select, use: [inline]}
dns:
  enable: true
  nameserver: [1.1.1.1]
rules:
  - MATCH,SELECT
`, true)
	if err == nil || !strings.Contains(err.Error(), "proxy-providers.inline.payload[0].hop-interval") {
		t.Fatalf("unsafe inline provider Hysteria hop interval error = %v", err)
	}
}

func TestParseConfigForIOSRejectsUnsafeHysteria2HopInterval(t *testing.T) {
	setupConfigPipelineTest(t)
	_, err := parseConfigForIOS(`
proxies:
  - name: HY2
...
`, true)
	if err == nil || !strings.Contains(err.Error(), "proxies[0].hop-interval") {
		t.Fatalf("unsafe Hysteria2 hop interval error = %v", err)
	}
}
...
func TestParseConfigForIOSRejectsUnsafeOutboundTransportDurations(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"TUIC heartbeat overflow": {
			proxy: "{name: TUIC, type: tuic, heartbeat-interval: 9223372036855}",
			field: "heartbeat-interval",
		},
		"TUIC v4 negative request timeout": {
			proxy: "{name: TUIC, type: tuic, token: fixture, request-timeout: -1}",
			field: "request-timeout",
		},
		"MASQUE handshake overflow": {
			proxy: "{name: MASQUE, type: masque, handshake-timeout: 9223372037}",
			field: "handshake-timeout",
		},
		"OpenVPN handshake overflow": {
			proxy: "{name: OVPN, type: openvpn, handshake-timeout: 9223372037}",
			field: "handshake-timeout",
		},
		"WireGuard negative refresh": {
			proxy: "{name: WG, type: wireguard, refresh-server-ip-interval: -1}",
			field: "refresh-server-ip-interval",
		},
	}
...
rules: [MATCH,DIRECT]
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe outbound duration error = %v", err)
			}
		})
	}
}
...
func TestParseConfigForIOSRejectsUnsafeNestedTransportDurations(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"VMess gRPC ping overflow": {
			proxy: "{name: VMESS, type: vmess, network: grpc, grpc-opts: {ping-interval: 9223372037}}",
			field: "grpc-opts.ping-interval",
		},
		"Trojan gRPC ping overflow": {
			proxy: "{name: TROJAN, type: trojan, network: grpc, grpc-opts: {ping-interval: 9223372037}}",
			field: "grpc-opts.ping-interval",
		},
		"VLESS gRPC ping overflow": {
			proxy: "{name: VLESS, type: vless, network: grpc, grpc-opts: {ping-interval: 9223372037}}",
			field: "grpc-opts.ping-interval",
		},
		"VLESS XHTTP keepalive overflow": {
			proxy: "{name: VLESS, type: vless, network: xhttp, xhttp-opts: {reuse-settings: {h-keep-alive-period: 9223372037}}}",
			field: "xhttp-opts.reuse-settings.h-keep-alive-period",
		},
		"VLESS XHTTP download keepalive overflow": {
			proxy: "{name: VLESS, type: vless, network: xhttp, xhttp-opts: {download-settings: {reuse-settings: {h-keep-alive-period: 9223372037}}}}",
			field: "xhttp-opts.download-settings.reuse-settings.h-keep-alive-period",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe nested transport duration error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeSessionTransportDurations(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"AnyTLS idle check overflow": {
			proxy: "{name: ANYTLS, type: anytls, idle-session-check-interval: 9223372037}",
			field: "idle-session-check-interval",
		},
		"AnyTLS idle timeout overflow": {
			proxy: "{name: ANYTLS, type: anytls, idle-session-timeout: 9223372037}",
			field: "idle-session-timeout",
		},
		"OpenVPN ping overflow": {
			proxy: "{name: OVPN, type: openvpn, ping: 9223372037}",
			field: "ping",
		},
		"OpenVPN ping restart overflow": {
			proxy: "{name: OVPN, type: openvpn, ping-restart: 9223372037}",
			field: "ping-restart",
		},
		"VMess Mekya polling overflow": {
			proxy: "{name: VMESS, type: vmess, network: mekya, mekya-opts: {polling-interval-initial: 9223372036855}}",
			field: "mekya-opts.polling-interval-initial",
		},
		"VMess Mekya write delay overflow": {
			proxy: "{name: VMESS, type: vmess, network: mekya, mekya-opts: {max-write-delay: 9223372036855}}",
			field: "mekya-opts.max-write-delay",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe session transport duration error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeKcptunPluginDurations(t *testing.T) {
	tests := map[string]struct {
		options string
		field   string
	}{
		"negative keepalive": {
			options: "{keepalive: -1}",
			field:   "plugin-opts.keepalive",
		},
		"keepalive overflow": {
			options: "{keepalive: 9223372037}",
			field:   "plugin-opts.keepalive",
		},
		"autoexpire overflow": {
			options: "{autoexpire: 9223372037}",
			field:   "plugin-opts.autoexpire",
		},
		"active scavengettl overflow": {
			options: "{autoexpire: 60, scavengettl: 9223372037}",
			field:   "plugin-opts.scavengettl",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe kcptun duration error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeTUICDatagramSizes(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"negative UDP relay packet size": {
			proxy: "{name: TUIC, type: tuic, max-udp-relay-packet-size: -1}",
			field: "max-udp-relay-packet-size",
		},
		"UDP relay packet size addition overflow": {
			proxy: "{name: TUIC, type: tuic, max-udp-relay-packet-size: 9223372036854775807}",
			field: "max-udp-relay-packet-size",
		},
		"v5 datagram frame smaller than packet header": {
			proxy: "{name: TUIC, type: tuic, max-datagram-frame-size: 1}",
			field: "max-datagram-frame-size",
		},
		"v4 datagram frame smaller than packet header": {
			proxy: "{name: TUIC, type: tuic, token: fixture, max-datagram-frame-size: 1}",
			field: "max-datagram-frame-size",
		},
	}
	for name, test := range tests {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe TUIC datagram size error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeQUICFlowControlWindows(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"Hysteria negative stream window": {
			proxy: "{name: HY, type: hysteria, recv-window-conn: -1}",
			field: "recv-window-conn",
		},
		"Hysteria oversized connection window": {
			proxy: "{name: HY, type: hysteria, recv-window: 4611686018427387904}",
			field: "recv-window",
		},
		"TUIC negative connection window": {
			proxy: "{name: TUIC, type: tuic, recv-window: -1}",
			field: "recv-window",
		},
		"Hysteria2 oversized initial stream window": {
			proxy: "{name: HY2, type: hysteria2, initial-stream-receive-window: 4611686018427387904}",
			field: "initial-stream-receive-window",
		},
		"Hysteria2 oversized max connection window": {
			proxy: "{name: HY2, type: hysteria2, max-connection-receive-window: 4611686018427387904}",
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe QUIC flow-control window error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeHysteriaRates(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"negative compatible upload speed": {
			proxy: "{name: HY, type: hysteria, up-speed: -1}",
			field: "up-speed",
		},
		"compatible download speed multiplication overflow": {
			proxy: "{name: HY, type: hysteria, down-speed: 73786976294839}",
			field: "down-speed",
		},
		"upload byte rate multiplication overflow": {
			proxy: "{name: HY, type: hysteria, up: '18446745 TBps'}",
			field: "up",
		},
		"download bit rate multiplication overflow": {
			proxy: "{name: HY, type: hysteria, down: '18446745 Tbps'}",
			field: "down",
		},
	}
	for name, test := range tests {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe Hysteria rate error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeHysteria2AndBrutalRates(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"Hysteria2 upload rate overflow": {
			proxy: "{name: HY2, type: hysteria2, server: 127.0.0.1, port: 443, password: fixture, up: '18446745 TBps'}",
			field: "up",
		},
		"sing-mux Brutal download rate overflow": {
			proxy: "{name: DIRECT-MUX, type: direct, smux: {enabled: true, brutal-opts: {enabled: true, down: '18446745 TBps'}}}",
			field: "smux.brutal-opts.down",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe outbound rate error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeBBRCongestionWindows(t *testing.T) {
	tests := map[string]struct {
		proxy string
		field string
	}{
		"Hysteria2 negative window": {
			proxy: "{name: HY2, type: hysteria2, server: 127.0.0.1, port: 443, password: fixture, cwnd: -1}",
			field: "cwnd",
		},
		"TUIC packet multiplication overflow": {
			proxy: "{name: TUIC, type: tuic, server: 127.0.0.1, port: 443, uuid: 00000000-0000-0000-0000-000000000001, password: fixture, congestion-controller: bbr_meta_v1, cwnd: 7205759403792794}",
			field: "cwnd",
		},
	}
	for name, test := range tests {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe BBR congestion window error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeHysteria2UDPMTU(t *testing.T) {
	setupConfigPipelineTest(t)
	_, err := parseConfigForIOS(`
proxies:
  - name: HY2
...
rules:
  - MATCH,DIRECT
`, true)
	if err == nil || !strings.Contains(err.Error(), "proxies[0].udp-mtu") {
		t.Fatalf("unsafe Hysteria2 UDP MTU error = %v", err)
	}
}

func TestParseConfigForIOSRejectsUnsafeTUICMaxOpenStreams(t *testing.T) {
	setupConfigPipelineTest(t)
	_, err := parseConfigForIOS(`
proxies:
  - name: TUIC
...
rules:
  - MATCH,DIRECT
`, true)
	if err == nil || !strings.Contains(err.Error(), "proxies[0].max-open-streams") {
		t.Fatalf("unsafe TUIC maximum open streams error = %v", err)
	}
}

func TestParseConfigForIOSRejectsUnboundedXHTTPGeneratedBuffers(t *testing.T) {
	tests := map[string]struct {
		xhttp string
		field string
	}{
		"padding": {
			xhttp: "x-padding-bytes: 4194305",
			field: "xhttp-opts.x-padding-bytes",
		},
		"custom session identifier": {
			xhttp: "session-table: Alphabet, session-length: 4194305",
			field: "xhttp-opts.session-length",
		},
		"stream-up post buffer": {
			xhttp: "sc-max-each-post-bytes: 4194305",
			field: "xhttp-opts.sc-max-each-post-bytes",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS("proxies:\n  - {name: VLESS, type: vless, server: 127.0.0.1, port: 443, uuid: 00000000-0000-0000-0000-000000000001, network: xhttp, xhttp-opts: {"+test.xhttp+`}}
dns:
  enable: true
  nameserver: [1.1.1.1]
rules:
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unbounded XHTTP generated buffer error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeXHTTPReuseRanges(t *testing.T) {
	tests := map[string]struct {
		reuse string
		field string
	}{
		"random range arithmetic overflow": {
			reuse: "max-concurrency: '0-9223372036854775807'",
			field: "xhttp-opts.reuse-settings.max-concurrency",
		},
		"request counter narrowing": {
			reuse: "h-max-request-times: 2147483648",
			field: "xhttp-opts.reuse-settings.h-max-request-times",
		},
		"reuse lifetime duration overflow": {
			reuse: "h-max-reusable-secs: 9223372037",
			field: "xhttp-opts.reuse-settings.h-max-reusable-secs",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS("proxies:\n  - {name: VLESS, type: vless, server: 127.0.0.1, port: 443, uuid: 00000000-0000-0000-0000-000000000001, network: xhttp, xhttp-opts: {reuse-settings: {"+test.reuse+`}}}
dns:
  enable: true
  nameserver: [1.1.1.1]
rules:
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe XHTTP reuse range error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeXHTTPPacketUpScheduling(t *testing.T) {
	tests := map[string]string{
		"random range arithmetic overflow": "'0-9223372036854775807'",
		"millisecond duration overflow":    "9223372036855",
	}
	for name, interval := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS("proxies:\n  - {name: VLESS, type: vless, server: 127.0.0.1, port: 443, uuid: 00000000-0000-0000-0000-000000000001, network: xhttp, xhttp-opts: {mode: packet-up, sc-min-posts-interval-ms: "+interval+`}}
dns:
  enable: true
  nameserver: [1.1.1.1]
rules:
  - MATCH,DIRECT
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0].xhttp-opts.sc-min-posts-interval-ms") {
				t.Fatalf("unsafe XHTTP packet-up scheduling error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeKcptunConnectionCounts(t *testing.T) {
	tests := map[string]int{
		"negative slice length":   -1,
		"uint16 round-robin wrap": 65536,
	}
	for name, count := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS(fmt.Sprintf(`
proxies:
  - name: SS
    type: ss
    server: 127.0.0.1
...
rules:
  - MATCH,DIRECT
`, count), true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0].plugin-opts.conn") {
				t.Fatalf("unsafe kcptun connection count error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeKcptunFECShards(t *testing.T) {
	tests := map[string]struct {
		options string
		field   string
	}{
		"negative data shards": {
			options: "datashard: -1, parityshard: 3",
			field:   "plugin-opts.datashard",
		},
		"negative parity shards": {
			options: "datashard: 10, parityshard: -1",
			field:   "plugin-opts.parityshard",
		},
		"default parity exceeds decoder total": {
			options: "datashard: 254",
			field:   "plugin-opts.parityshard",
		},
		"uint32 total wraps encoder divisor": {
			options: "datashard: 2147483648, parityshard: 2147483648",
			field:   "plugin-opts.datashard",
		},
	}
	for name, test := range tests {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe kcptun FEC shard error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeKcptunSmuxSettings(t *testing.T) {
	tests := map[string]struct {
		options string
		field   string
	}{
		"negative version reaches VerifyConfig": {
			options: "smuxver: -1",
			field:   "plugin-opts.smuxver",
		},
		"receive buffer narrows to int32": {
			options: "smuxbuf: 2147483648",
			field:   "plugin-opts.smuxbuf",
		},
		"frame exceeds wire length": {
			options: "framesize: 65536",
			field:   "plugin-opts.framesize",
		},
		"stream exceeds receive buffer": {
			options: "smuxbuf: 1024, streambuf: 1025",
			field:   "plugin-opts.streambuf",
		},
		"stream buffer narrows on wire": {
			options: "smuxbuf: 2147483647, streambuf: 2147483648",
			field:   "plugin-opts.streambuf",
		},
		"keepalive timeout multiplication overflows": {
			options: "keepalive: 3074457346",
			field:   "plugin-opts.keepalive",
		},
	}
	for name, test := range tests {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe kcptun smux setting error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeKcptunTransportSettings(t *testing.T) {
	tests := map[string]struct {
		options string
		field   string
	}{
		"classic cipher MTU too small": {
			options: "mtu: 77",
			field:   "plugin-opts.mtu",
		},
		"AEAD cipher MTU too small": {
			options: "crypt: aes-128-gcm, mtu: 85",
			field:   "plugin-opts.mtu",
		},
		"null cipher MTU too small": {
			options: `crypt: "null", mtu: 57`,
			field:   "plugin-opts.mtu",
		},
		"negative rate limit narrows to uint32": {
			options: "ratelimit: -1",
			field:   "plugin-opts.ratelimit",
		},
		"rate limit wraps to disabled": {
			options: "ratelimit: 4294967296",
			field:   "plugin-opts.ratelimit",
		},
		"receive window wraps wire field": {
			options: "rcvwnd: 65536",
			field:   "plugin-opts.rcvwnd",
		},
		"negative send window is ignored": {
			options: "sndwnd: -1",
			field:   "plugin-opts.sndwnd",
		},
		"socket buffer narrows in Darwin syscall": {
			options: "sockbuf: 2147483648",
			field:   "plugin-opts.sockbuf",
		},
		"DSCP exceeds six-bit field": {
			options: "dscp: 64",
			field:   "plugin-opts.dscp",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
...
`, true)
			if err == nil || !strings.Contains(err.Error(), "proxies[0]."+test.field) {
				t.Fatalf("unsafe kcptun transport setting error = %v", err)
			}
		})
	}
}

func TestParseConfigForIOSRejectsUnsafeGlobalAndDNSDurations(t *testing.T) {
	tests := map[string]struct {
		config string
		field  string
	}{
		"DNS IPv6 timeout overflow": {
			config: "dns:\n  enable: true\n  ipv6-timeout: 9223372036855\n  nameserver: [1.1.1.1]\n",
			field:  "dns.ipv6-timeout",
		},
		"TCP keepalive idle overflow": {
			config: "keep-alive-idle: 9223372037\ndns:\n  enable: true\n  nameserver: [1.1.1.1]\n",
			field:  "keep-alive-idle",
		},
		"TCP keepalive interval overflow": {
			config: "keep-alive-interval: 9223372037\ndns:\n  enable: true\n  nameserver: [1.1.1.1]\n",
			field:  "keep-alive-interval",
		},
		"enabled NTP interval overflow": {
			config: "ntp:\n  enable: true\n  interval: 153722868\ndns:\n  enable: true\n  nameserver: [1.1.1.1]\n",
			field:  "ntp.interval",
		},
		"TUN UDP timeout overflow": {
			config: "tun:\n  udp-timeout: 9223372037\ndns:\n  enable: true\n  nameserver: [1.1.1.1]\n",
			field:  "tun.udp-timeout",
		},
		"TUN ICMP timeout overflow": {
			config: "tun:\n  icmp-timeout: 9223372037\ndns:\n  enable: true\n  nameserver: [1.1.1.1]\n",
			field:  "tun.icmp-timeout",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS(test.config+"rules:\n  - MATCH,DIRECT\n", true)
			if err == nil || !strings.Contains(err.Error(), test.field) {
				t.Fatalf("unsafe global/DNS duration error = %v", err)
			}
		})
	}
}
...
func TestParseConfigForIOSRejectsUnsafeDNSCacheControls(t *testing.T) {
	tests := map[string]struct {
		dns   string
		field string
	}{
		"negative cache capacity": {
			dns:   "cache-max-size: -1",
			field: "dns.cache-max-size",
		},
		"fake-IP TTL exceeds DNS wire field": {
			dns:   "enhanced-mode: fake-ip\n  fake-ip-ttl: 4294967296",
			field: "dns.fake-ip-ttl",
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			setupConfigPipelineTest(t)
			_, err := parseConfigForIOS("dns:\n  enable: true\n  nameserver: [1.1.1.1]\n  "+test.dns+`
rules:
  - MATCH,DIRECT
`, true)
			if err == nil || !strings.Contains(err.Error(), test.field) {
				t.Fatalf("unsafe DNS cache control error = %v", err)
			}
		})
	}
}

...
func NewService(platform PlatformInterface) (*BoxService, error) {
	if platform == nil {
		return nil, errors.New("hako: NewService requires a platform")
	}
	setupMu.Lock()
	ok := setupDone
	...
	setupMu.Unlock()
	if !ok {
		return nil, errors.New("hako: call Setup before NewService")
	}
	if cacheDisabled {
		return nil, errors.New("hako: NewService is unavailable after App-only DisablePersistentCache Setup")
	}
...
func (s *BoxService) Start(configContent string) (startErr error) {
	...
	go func() {
		...
		cfg, runtime, err := parseConfigForIOSRuntime(configContent, s.platform.UnderNetworkExtension())
...
func (s *BoxService) Reload(configContent string) error {
	...
	defer s.mu.Unlock()
	if !s.running {
		return errors.New("hako: Reload before Start")
	}
	done := make(chan error, 1)
	go func() {
		...
		underNE := s.platform.UnderNetworkExtension()
		cfg, runtime, err := parseConfigForIOSRuntime(configContent, underNE)
		if err != nil {
			done <- err
			return
		}
...
func (s *BoxService) RuntimeDiagnosticsJSON() string {
	...
	// Live gVisor TCP window state (nil hook = no gVisor stack running). The
	// range is read back from the stack itself so evidence can prove which
	// window a run really used; the distribution shows how far moderation
	// actually grew busy connections.

...
// geodata-mode is off unless the configuration says otherwise, and off means the tunnel
// answers GEOIP from geoip.metadb (rules/common/geoip.go) and never opens compiled-geoip.
// Compiling for it was a wasted decode -- and a wrong one: the preflight has just called
// C.Path.MMDB(), which renames the process-wide C.GeoipName to "geoip.metadb", so the
// compile read the MMDB as protobuf, failed with "cannot parse invalid wire-format data",
// and warned that the country would match nothing. It matched fine (found in a device log,
// 2026-09-02); the warning was about a file the tunnel does not read.
func TestPrepareGeoIPCacheSkipsWhenTheTunnelReadsMMDB(t *testing.T) {
	options := testOptions(t)
	if err := os.MkdirAll(options.WorkingPath, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	// The App's order: the preflight has looked the mmdb up and the name has leaked.
	_ = C.Path.MMDB()
	...
	if summary != "geoip: skipped, tunnel reads geoip.metadb" {
		t.Fatalf("the App did not say it skipped a compile nothing would read: %q", summary)
	}
	path, err := compiled.IPCIDRPath(geodata.CompiledGeoIPDir(), "cn")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(path); err == nil {
		t.Fatalf("an artifact was written for a tunnel that reads geoip.metadb: %s", path)
	}
}

// The client decides whether to run the compile pass from the same key, decoded the way
// the plan decodes it (plan_resources.go stages GeoIP.dat or geoip.metadb from
// raw.GeodataMode), so the two cannot disagree about which file the tunnel will read.

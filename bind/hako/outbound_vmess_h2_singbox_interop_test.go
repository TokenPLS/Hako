...
func TestControlledVMessHTTP2SingBoxReferenceInterop(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("generate sing-box VMess HTTP/2 certificate: %v", err)
	}
	...
	if err := os.WriteFile(certificatePath, []byte(certificate), 0o600); err != nil {
		t.Fatalf("write sing-box VMess HTTP/2 certificate: %v", err)
	}
	if err := os.WriteFile(privateKeyPath, []byte(privateKey), 0o600); err != nil {
		t.Fatalf("write sing-box VMess HTTP/2 private key: %v", err)
	}
	uuid := utils.NewUUIDV4().String()
	const (
		serverName = "controlled-vmess-h2.example"
		path       = "/controlled-vmess-h2"
	)
	serverPort := reserveControlledTCPPort(t)
	serverConfig, err := json.Marshal(map[string]any{
		"log": map[string]any{"level": "trace", "timestamp": false},
		"inbounds": []map[string]any{{
			"type":        "vmess",
			"tag":         "controlled-vmess-h2-in",
			"listen":      "127.0.0.1",
	...
	mapping := map[string]any{
		...
		"udp":         true,
		"h2-opts":     map[string]any{"host": []string{serverName}, "path": path},
	}
	...
	cancel()
	if err != nil {
		t.Fatalf("VMess HTTP/2 sing-box URLTest() error = %v\nsing-box logs:\n%s", err, serverLogs.String())
	}
	if delay == 0 || targetRequests.Load() != 1 {
		t.Fatal("VMess HTTP/2 sing-box request did not reach the target")
	}

	udpTarget, err := net.ListenUDP("udp4", &net.UDPAddr{IP: net.IPv4(127, 0, 0, 1)})
	if err != nil {
		t.Fatalf("listen for VMess HTTP/2 UDP echo target: %v", err)
	}
	...
	cancel()
	if err != nil {
		t.Fatalf("VMess HTTP/2 sing-box UDP session failed: %v\nsing-box logs:\n%s", err, serverLogs.String())
	}
	if err := packetConnection.SetDeadline(time.Now().Add(5 * time.Second)); err != nil {
		_ = packetConnection.Close()
		t.Fatalf("VMess HTTP/2 sing-box UDP deadline failed: %v", err)
	}
	payload := []byte("hako-controlled-vmess-h2-sing-box-udp")
	if _, err := packetConnection.WriteTo(payload, udpAddress); err != nil {
		_ = packetConnection.Close()
		t.Fatalf("VMess HTTP/2 sing-box UDP write failed: %v", err)
	}
	buffer := make([]byte, 64*1024)
	count, source, err := packetConnection.ReadFrom(buffer)
	closeError := packetConnection.Close()
	if err != nil {
		t.Fatalf("VMess HTTP/2 sing-box UDP read failed: %v\nsing-box logs:\n%s", err, serverLogs.String())
	}
	if closeError != nil && !errors.Is(closeError, net.ErrClosed) {
		t.Fatalf("VMess HTTP/2 sing-box UDP close failed: %v", closeError)
	}
	if !bytes.Equal(buffer[:count], payload) || source.String() != udpAddress.String() {
		t.Fatal("VMess HTTP/2 sing-box UDP response did not match the request")
	}

	_, _, wrongFingerprint, err := ca.NewRandomTLSKeyPair(ca.KeyPairTypeP256)
	if err != nil {
		t.Fatalf("generate wrong VMess HTTP/2 fingerprint: %v", err)
	}
	for _, failure := range []struct {
		...
		t.Run(failure.name, func(t *testing.T) {
			...
			failedMapping["name"] = "controlled-vmess-h2-sing-box-" + failure.name
			...
			cancel()
			if err == nil {
				t.Fatalf("VMess HTTP/2 sing-box %s unexpectedly succeeded", failure.name)
			}
			if failure.wantMessage != "" && !strings.Contains(strings.ToLower(err.Error()), failure.wantMessage) {
				t.Fatalf("VMess HTTP/2 sing-box failure error = %v, want %q", err, failure.wantMessage)
			}
			if count := targetRequests.Load(); count != 1 {
				t.Fatalf("VMess HTTP/2 sing-box failed attempt leaked to target; request count = %d", count)
			}
		})
	}
}

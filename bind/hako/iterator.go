...
func WrapString(value string) *StringBox {
	// The only place a StringBox is built, and it repairs the value on the
	// way in: the Value field getter crosses the bridge under a nonnull
	// annotation, so a box must never hold bytes that decode to nil there.

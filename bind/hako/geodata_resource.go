...
const maximumGeodataResourceBytes int64 = 128 * 1024 * 1024
...
func validateGeodataFile(kind, format, path string) (string, string, []byte, error) {
	...
	payload, err := readBoundedRegularFile(path, maximumGeodataResourceBytes, "geodata")
	if err != nil {
		return "", "", nil, err
	}
	switch {
...
func ValidateGeodataCodesForIOS(kind, format, path, codes string) error {
	...
	if normalizedKind == "geoip" {
		return validateGeoIPDatCodes(payload, wanted)
	}
	return validateGeoSiteDatCodes(payload, wanted)
}

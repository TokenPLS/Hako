...
func CheckConfig(configContent string) error {
	...
	setupMu.Unlock()
	if !ready {
		return fmt.Errorf("hako: call Setup before CheckConfig")
	}
...
func ValidateConfigShape(configContent string) error {
	...
	if _, err := config.UnmarshalRawConfig([]byte(configContent)); err != nil {
		return fmt.Errorf("hako: parse config: %w", err)
	}
	return nil
}
...
func PlatformConfigIntentJSON(configContent string) (*StringBox, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("hako: encode platform config intent: %w", err)
	}
...
func FormatConfig(configContent string) (*StringBox, error) {
	...
	if err := yaml.Unmarshal([]byte(configContent), &document); err != nil {
		return nil, fmt.Errorf("hako: parse YAML for formatting: %w", err)
	}
	var buffer bytes.Buffer
	encoder := yaml.NewEncoder(&buffer)
	encoder.SetIndent(2)
	if err := encoder.Encode(&document); err != nil {
		return nil, fmt.Errorf("hako: format config: %w", err)
	}
	if err := encoder.Close(); err != nil {
		return nil, fmt.Errorf("hako: finish config formatting: %w", err)
	}

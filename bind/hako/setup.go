...
func Setup(options *SetupOptions) error {
	...
	if options == nil {
		return errors.New("hako: Setup called with nil options")
	}
	if options.BasePath == "" || options.WorkingPath == "" || options.TempPath == "" {
		return errors.New("hako: SetupOptions.BasePath, WorkingPath and TempPath are required")
	}
	...
	if err != nil {
		return fmt.Errorf("hako: SetupOptions.CertificateStore: %w", err)
	}
	...
	if options.DisablePersistentCache {
		if err := cachefile.DisablePersistentCache(); err != nil {
			return fmt.Errorf("hako: disable persistent cache: %w", err)
		}
	...
	if activeCoreCount.Load() > 0 && requestedTunMTU != effectiveTunMTU() {
		return fmt.Errorf("hako: changing TunMTU from %d to %d requires restart", effectiveTunMTU(), requestedTunMTU)
	}
	...
	if options.TimeZone != "" {
		if err := applyTimeZone(options.TimeZone); err != nil {
			return fmt.Errorf("hako: apply timezone %q: %w", options.TimeZone, err)
		}
	}
	...
	if legacyClashAPIPath != setupClashAPIPath {
	...
	armMemoryPressureMonitorForRuntime(
		...
		options.DisablePersistentCache,
		func() {
			startPressureThresholdMonitor(currentRuntimeSetup.softMemoryLimit, options.MemoryPressureShed)
		},
	)

...
func parseConfigForIOSRuntime(content string, underNE bool) (*config.Config, *providerRuntime, error) {
...
func parseConfigForIOSInternal(content string, underNE bool, stageRuntime bool) (*config.Config, *providerRuntime, error) {
	...
	if err := validateForApple(cfg, policy); err != nil {
		if runtime != nil {
			runtime.close()
		}
		return nil, nil, err
...
func normalizeRawConfigForApple(raw *config.RawConfig, policy appleRuntimePolicy) {
	...
	if policy.networkExtension {
		...
		for _, ns := range stripNEIncompatibleNameservers(raw) {
			log.Warnln("[Apple %s] DNS %s cannot work from a packet tunnel (system:// resolves back through NEDNSSettings; dhcp:// must bind 0.0.0.0:68 on a physical interface); stripped, resolution stays inside the core and the config still starts", policy.profile.String(), ns)
		}
...
// redactDNSServerForDiagnostics renders a nameserver for a diagnostic that
// reaches two product exits -- log.Warnln (config_pipeline) and the plan's
// App-facing notices (plan_resources). A DoH/DoT nameserver can carry
// credentials in userinfo (user:pass@), the query (?token=) or even the path (a
// NextDNS profile id lives there), and a product log may hold none of them
// (AGENTS.md §6). It keeps the scheme, host and the extracted fragment proxy
// name -- the two facts the reader needs to find the misrouted entry -- and
// drops everything that can hide a secret. This is an EXIT constraint: the
// nameserver stays verbatim in the config; only the diagnostic string changes.
func redactDNSServerForDiagnostics(server string) string {
	suffix := ""
	if name := dnsFragmentProxyName(server); name != "" {
		suffix = "#" + name
	}
	u, err := url.Parse(server)
	if err != nil || u.Host == "" {
		// Bare-host or unparseable forms (8.8.8.8#name, a scheme without //).
		// These carry no userinfo; keep them only when nothing in the string can
		// hide a secret, otherwise fall back to the fragment name alone.
...
func detectUnroutableDNSFragments(raw *config.RawConfig) []string {
	...
	rewrite := func(field string, list []string) {
		for index, server := range list {
			...
			stripped = append(stripped, field+" "+redactDNSServerForDiagnostics(server))
			_ = index
		}
	}
	...
	for field, policy := range map[string]*orderedmap.OrderedMap[string, any]{
		...
		for pair := policy.Oldest(); pair != nil; pair = pair.Next() {
			for _, server := range dnsServerStrings(pair.Value) {
				...
				stripped = append(stripped, field+" "+pair.Key+" "+redactDNSServerForDiagnostics(server))
			}
		}
	}
	return stripped
...
func validateRawConfigForIOS(raw *config.RawConfig) error {
	if err := validateRawGlobalDurationsForIOS(raw); err != nil {
		return err
	}
	if field, reason := firstUnsafeOutboundRuntimeOption(raw); field != "" {
		return fmt.Errorf("hako: %s: %s", field, reason)
	}
	if err := validateRawProxyGroupDurationsForIOS(raw.ProxyGroup); err != nil {
		return err
	}
...
func validateRawGlobalDurationsForIOS(raw *config.RawConfig) error {
	for _, field := range []struct {
		name  string
		value any
	}{
		{name: "keep-alive-idle", value: raw.KeepAliveIdle},
		{name: "keep-alive-interval", value: raw.KeepAliveInterval},
	} {
		if _, err := providerNonPositiveDurationAllowedUnits(field.value, time.Second, "TCP "+field.name, "second"); err != nil {
			return fmt.Errorf("hako: %s: %w", field.name, err)
		}
	}
	if raw.DNS.Enable {
		if _, err := providerDurationUnits(raw.DNS.IPv6Timeout, time.Millisecond, "DNS IPv6 timeout", "millisecond"); err != nil {
			return fmt.Errorf("hako: dns.ipv6-timeout: %w", err)
		}
		if raw.DNS.CacheMaxSize < 0 {
			return fmt.Errorf("hako: dns.cache-max-size must not be negative")
		}
		if raw.DNS.EnhancedMode != C.DNSNormal && raw.DNS.FakeIPTTL > math.MaxUint32 {
			return fmt.Errorf("hako: dns.fake-ip-ttl exceeds the DNS uint32 TTL field")
		}
	}
	if raw.NTP.Enable {
		if _, err := providerNonPositiveDurationAllowedUnits(raw.NTP.Interval, time.Minute, "NTP update interval", "minute"); err != nil {
			return fmt.Errorf("hako: ntp.interval: %w", err)
		}
	}
	for _, field := range []struct {
		name  string
		...
		{name: "udp-timeout", value: raw.Tun.UDPTimeout},
		{name: "icmp-timeout", value: raw.Tun.ICMPTimeout},
	} {
		if _, err := providerNonPositiveDurationAllowedUnits(field.value, time.Second, "TUN "+field.name, "second"); err != nil {
			return fmt.Errorf("hako: tun.%s: %w", field.name, err)
		}
	}
	return nil
}

func validateRawProxyGroupDurationsForIOS(groups []map[string]any) error {
	fields := []struct {
		name      string
		unit      time.Duration
		unitLabel string
		...
		{name: "timeout", unit: time.Millisecond, unitLabel: "millisecond"},
	}
	for index, group := range groups {
		for _, field := range fields {
			if _, err := providerDurationUnits(group[field.name], field.unit, "proxy group "+field.name, field.unitLabel); err != nil {
				return fmt.Errorf("hako: proxy-groups[%d].%s: %w", index, field.name, err)
			}
		}
	}
	return nil

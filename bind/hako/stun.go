...
func FormatNATMapping(value int32) string {
	return internalSTUN.NATMapping(value).String()
}

func FormatNATFiltering(value int32) string {
	return internalSTUN.NATFiltering(value).String()
}
...
func (c *ClashAPIClient) StartSTUNTest(server, outbound string, handler STUNTestHandler) (*STUNTestSession, error) {
	if c == nil || c.httpClient == nil {
		return nil, errors.New("hako: Clash API client is unavailable")
	}
	if handler == nil {
		return nil, errors.New("hako: STUN test requires a handler")
	}
	payload, err := json.Marshal(stunWireRequest{Server: server, Outbound: outbound})
	if err != nil {
		return nil, fmt.Errorf("hako: encode STUN request: %w", err)
	}
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("hako: build STUN request: %w", err)
	}
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("hako: start STUN test: %w", err)
	}
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		defer response.Body.Close()
		message, _ := io.ReadAll(io.LimitReader(response.Body, 4097))
		cancel()
		return nil, fmt.Errorf("hako: start STUN test: HTTP %d: %s", response.StatusCode, strings.TrimSpace(string(message)))
	}
	...
	if mediaTypeError != nil || mediaType != "application/x-ndjson" {
		...
		cancel()
		return nil, errors.New("hako: STUN stream returned an unexpected content type")
	}

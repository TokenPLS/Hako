...
func (p *Proxy) URLTest(ctx context.Context, url string, expectedStatus utils.IntRanges[uint16]) (t uint16, err error) {
	...
	// A valid local/edge response can complete in less than one millisecond.
	// Zero is the controller's failure sentinel, so preserve a successful
	// measurement as the minimum representable delay.
	if t == 0 {
		t = 1
	}
	if !satisfied {
		err = fmt.Errorf("unexpected HTTP status %d from URL test target", resp.StatusCode)
	}
	return
}

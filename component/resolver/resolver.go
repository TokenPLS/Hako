...
func eachConfiguredResolver(do func(Resolver)) {
	// Deduplicated by identity, because the four are not four distinct objects.
	// DefaultResolver is assigned the whole dns.Resolvers aggregate, and
	// ProxyServerHostResolver and DirectHostResolver are assigned that aggregate's own
	// ProxyResolver and DirectResolver -- whose ClearCache and ResetConnection the
	// aggregate's methods already fan out to. Walking all four blindly therefore clears
	// and resets the same resolvers twice. Both operations are idempotent so it was
	// wasteful rather than wrong, but the entire point of gating these calls is to stop
	// doing them when they are not needed, and doing each one twice contradicts that.
	//
	...
	seen := make(map[Resolver]struct{}, 4)
	for _, r := range []Resolver{DefaultResolver, ProxyServerHostResolver, DirectHostResolver, SystemResolver} {
		if r == nil {
			continue
		}

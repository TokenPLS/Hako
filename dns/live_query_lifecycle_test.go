...
func TestShutdownStillCancelsRefetches(t *testing.T) {
	...
	resolver := &Resolver{
		main:  []dnsClient{client},
		cache: Config{}.newCache(),
		// Optimistic serving is off by default now, so a test that needs a stale hit to
		// be SERVED has to arm it explicitly. Left at the default this seeds an entry that is a
		// hard miss, no refresh fires, and the test would be asserting nothing.
	}
	...
	// Inside the optimistic window, so it is served stale and fires a refresh rather than
	// being dropped as a hard miss.
	...
	if _, err := resolver.ExchangeContext(context.Background(), question); err != nil {
		t.Fatalf("a stale hit inside the optimistic window must still be served: %v", err)
	}

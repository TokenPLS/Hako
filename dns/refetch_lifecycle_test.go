...
//
// The stale window itself is not a subject here any more. An earlier revision of this
// comment argued about it against sing-box's optimistic mode; sing-box is not this fork's
// upstream. mihomo serves an expired entry unconditionally -- TTL rewritten to 1,
// refresh in the background, no window, no eviction -- restored exactly that
// shape, deleting the bounded window and its knobs. What this file tests is only who is
// allowed to end the background refresh.
...
func TestShutdownCancelsInFlightRefetches(t *testing.T) {
	...
	resolver := &Resolver{
		...
		// Nothing to arm: mihomo serves a stale hit and fires the background refresh
		// unconditionally, so the seeded expired entry below
		// is served and its refresh is the thing this test then cancels.
	}

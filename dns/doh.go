...
func newDoHClient(urlString string, r resolver.Resolver, preferH3 bool, params map[string]string, proxyAdapter C.ProxyAdapter, proxyName string) dnsClient {
	...
	doh := &dnsOverHTTPS{
		url: u,
		// Redacted, because addr is what Address() returns and Address() is
		// printed: log lines, diagnostics handed to the containing app, and
		// error text. A DoH URL authenticates with its userinfo, its query or
		// its path (a NextDNS profile id is the whole credential), and none of
		// those may reach a product log. The URL the transport actually dials
		// is doh.url, which keeps everything.
		addr:   redactedDNSURL(u),
...
// redactedDNSURL renders a resolver URL for anything that prints it.
//
// A DoH/DoQ nameserver can authenticate three ways and all of them live in the
// URL: userinfo (https://user:pass@…), the query (?token=…), and the path -- a
// NextDNS profile id is the entire credential. url.URL.String renders every one
// of them, and this fork's logs reach a file on the reader's device and an
// in-app diagnostics view, where none of them belong. Scheme and host are what
// a reader needs to know which upstream answered, so those stay.
func redactedDNSURL(u *url.URL) string {
	if u == nil {
		return ""
	}
	if u.Host == "" {
...
func (doh *dnsOverHTTPS) createTransport(ctx context.Context) (t http.RoundTripper, err error) {
	...
	if slices.Contains(doh.httpVersions, C.HTTPVersion3) {
		...
		if err == nil {
			log.Debugln("[%s] using HTTP/3 for this upstream: QUIC was faster", redactedDNSURL(doh.url))
			return transportH3, nil
		}
	}

	log.Debugln("[%s] using HTTP/2 for this upstream: %v", redactedDNSURL(doh.url), err)
